ui.m2_item_links.form = Ext.extend(Ext.form.FormPanel, {
	formWidth: 400,
	formHeight: 300,
	...
	constructor: function(config){
		...
		Ext.apply(this, {
			...
			items: [
				...
				{name: 'linked_item_id', xtype: 'hidden'},
				{fieldLabel:this.lblTitle, name: 'title', xtype: 'textfield'},
				{fieldLabel: this.lblType, hiddenName: 'type', xtype: 'combo',
						valueField: 'id', displayField: 'title', value: '1', emptyText: 'Выберите тип связки', 
						store: new Ext.data.JsonStore({url: 'di/m2_item_link_types/type_list.json', root: 'records', fields: ['id', 'title'], autoLoad: true,
							listeners: {
				...
				{fieldLabel: this.lblCategory, xtype: 'compositefield', items: [
					{xtype: 'button', iconCls: 'add', listeners: {click: function(){this.fireEvent('select_item')}, scope: this}},
		...
		ui.m2_item_links.form.superclass.constructor.call(this, config);
		this.on({
			...
			select_item: function(){
				...
				app.on({
					apploaded: function(){
						var bf = this.getForm();
						var f = new ui.m2_item.item_selection();
						var w = new Ext.Window({title: "Выбор", maximizable: true, modal: true, layout: 'fit', width: 640, height: 480, items: f});
						f.on({
							selected: function(data){
								bf.setValues([
									{id: 'linked_item_id', value: data.item_id},
									{id: 'title', value: data.item_title}
								]);
								w.close();
							},
							scope: this
				...
				app.Load('m2_item', 'item_selection');
			},
			scope: this
		})
	},
