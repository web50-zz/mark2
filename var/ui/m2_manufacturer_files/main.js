ui.m2_manufacturer_files.main = Ext.extend(ui.m2_manufacturer_files.grid, {
	Load: function(data){
		this.setParams({}, true);
	},
	Add: function(){
		...
		app.on({
			apploaded: function(){
				var f = new ui.m2_manufacturer_files.item_form();
		...
		app.Load('m2_manufacturer_files', 'item_form');
	},
	Edit: function(){
		var app = new App({waitMsg: 'Загрузка формы'});
		app.on({
	...
	constructor: function(config)
	{
		...
		Ext.apply(this, {
			...
			tbar: [
				...
				'->', {iconCls: 'help', handler: function(){showHelp('m2_manufacturer_files')}}
			]
		});
		Ext.apply(this, {
			listeners: {
				...
				rowmove: function(target, row){
					Ext.Ajax.request({
						url: 'di/m2_manufacturer_files/reorder.do',
						method: 'post',
		...
		ui.m2_manufacturer_files.main.superclass.constructor.call(this, config);
	},
