ui.m2_item_manufacturer.form = Ext.extend(Ext.form.FormPanel, {
	formWidth: 400,
	formHeight: 150,
	...
	Load: function(data){
		var f = this.getForm();
		f.load({
			url: 'di/m2_item_manufacturer/get.json',
	...
	Save: function(){
		...
		if (f.isValid()){
			f.submit({
				url: 'di/m2_item_manufacturer/set.do',
	...
	constructor: function(config){
		...
		Ext.apply(this, {
			...
			items: [
				...
				{fieldLabel: this.lblManufacturer, hiddenName: 'manufacturer_id', xtype: 'combo',
						valueField: 'id', displayField: 'title', value: '', emptyText: '', 
						store: new Ext.data.JsonStore({url: 'di/m2_manufacturers/available_list.json', root: 'records', fields: ['id', 'title'], autoLoad: true,
							listeners: {
								load: function(store,ops){
									var f = this.getForm().findField('manufacturer_id');
		...
		ui.m2_item_manufacturer.form.superclass.constructor.call(this, config);
		this.on({
