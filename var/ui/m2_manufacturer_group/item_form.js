ui.m2_manufacturer_group.item_form = Ext.extend(Ext.form.FormPanel, {
	formWidth: 600,
	formHeight: 320,

	fldTitle: 'Наименование',
	fldComment: 'Комментарий',
	fldMailFrom: 'E-Mail от',
	fldfollowupenabled:'Уведомление о регистрации',
	fldRegMsg:'Текст уведомления',
	...
	Load: function(data){
		if (data){
			var f = this.getForm();
			f.load({
				url: 'di/m2_manufacturer_group/get.json',
				params: {_sid: data.id},
	...
	Save: function(){
		...
		if (f.isValid()){
			f.submit({
				url: 'di/m2_manufacturer_group/set.do',
	...
	constructor: function(config){
		Ext.apply(this, {
			...
			items: [
				...
				{fieldLabel: this.fldMailFrom, name: 'mail_from', maxLength: 255},
		...
		ui.m2_manufacturer_group.item_form.superclass.constructor.call(this, config);
		this.on({
