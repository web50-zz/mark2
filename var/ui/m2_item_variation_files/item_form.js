ui.m2_item_variation_files.item_form = Ext.extend(Ext.form.FormPanel, {
	Load: function(data){
		var f = this.getForm();
		f.load({
			url: 'di/m2_item_variation_files/get.json',
			params: {_sid: data.id},
	...
	Save: function(){
		...
		if (f.isValid()){
			f.submit({
				url: 'di/m2_item_variation_files/set.do',
	...
	constructor: function(config){
		...
		Ext.apply(this, {
			...
			items: [
				...
				{fieldLabel: this.lblType, hiddenName: 'file_type', xtype: 'combo',
						valueField: 'id', displayField: 'title', value: '', emptyText: '', 
						store: new Ext.data.JsonStore({url: 'di/m2_file_types/type_list.json', root: 'records', fields: ['id', 'title'], autoLoad: true,
							listeners: {
		...
		ui.m2_item_variation_files.item_form.superclass.constructor.call(this, config);
		this.on({
	...
	filesList: function(){
		...
		app.on({
			apploaded: function(){
				var f = new ui.m2_item_variation_files.main();
		...
		app.Load('m2_item_variation_files', 'main');
