ui.m2_manufacturer_in_groups.subscribers = Ext.extend(ui.m2_manufacturer_in_groups.grid, {
	titleAdd: 'Выбор',
	...
	Selector: function(){
		...
		app.on({
			apploaded: function(){
				var f = new ui.m2_manufacturers.choice();
		...
		app.Load('m2_manufacturers', 'choice');
	},
	Add: function(contacts, id, cid, record){
		...
		{
			Ext.Ajax.request({
				url: 'di/m2_manufacturer_in_groups/add.do',
	...
	addAll: function(store){
		...
		{
			Ext.Ajax.request({
				url: 'di/m2_manufacturer_in_groups/add_all.do',
	...
	removeAll: function(store){
		...
		{
			Ext.Ajax.request({
				url: 'di/m2_manufacturer_in_groups/remove_all.do',
				params: store.baseParams,
				disableCaching: true,
				callback: function(options, success, response){
					var d = Ext.util.JSON.decode(response.responseText);
	...
	Delete: function(){
		...
		Ext.Msg.confirm(this.cnfrmTitle, this.cnfrmMsg, function(btn){
			if (btn == "yes"){
				...
				Ext.Ajax.request({
					url: 'di/m2_manufacturer_in_groups/unset.do',
					params: {'_sid[]': id},
					disableCaching: true,
					callback: function(options, success, response){
						var d = Ext.util.JSON.decode(response.responseText);
	...
	constructor: function(config)
	{
		...
		ui.m2_manufacturer_in_groups.subscribers.superclass.constructor.call(this, config);
	},
