...
class di_m2_chars extends data_interface
{
	...
	*/
	protected function sys_reorder()
	{
		...
		if ($opos < $npos)
			$query = "UPDATE `{$this->name}` SET `order` = IF(`id` = :id, :npos, `order` - 1) WHERE `order` >= :opos AND `order` <= :npos AND `m2_chars_type_id` = :pid";
		else
			$query = "UPDATE `{$this->name}` SET `order` = IF(`id` = :id, :npos, `order` + 1) WHERE `order` >= :npos AND `order` <= :opos AND `m2_chars_type_id` = :pid";
...
//			$args['order'] = $this->get_new_order((int)$this->get_args('m2_category_id'));
	...
	*/
	...
	{
		$this->_flush();
		$this->_get("SELECT MAX(`order`) + 1 AS `order` FROM `{$this->name}` WHERE `m2_chars_type_id` = {$pid}");
